    ...
    for i, file_paths in enumerate([files]):
        if i >= 0:
            try:
                ...
                df2 = df.select("url",extract_soup_udf(F.col("content")).alias("hi"))
                df2 = df2.select(F.col("url"),F.col("hi.a1").alias("a1"), F.col("hi.a2").alias("a2"),F.col("hi.wiki_url").alias("wiki_url"),F.col("hi.text1").alias("text1"), F.col("hi.text2").alias("text2")).filter(F.size("hi.wiki_url") > 0)
                df2 = df2.withColumn("new", F.arrays_zip("a1","a2","wiki_url","text1","text2"))
                df2 = df2.withColumn("new",F.explode("new"))
                df2 = df2.select(F.col("url"),F.col("new.a1").alias("a1"), F.col("new.a2").alias("a2"), F.col("new.wiki_url").alias("wiki_url"), F.col("new.text1").alias("text1"), F.col("new.text2").alias("text2"))
